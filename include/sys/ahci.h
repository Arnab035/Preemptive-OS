...
#define DMA_WRITE_EXT    0x35
#define DMA_READ_EXT     0x25
...
void find_first_disk(hba_mem_t *);

int find_type(hba_port_t *);

void init_ahci(hba_mem_t *);
...
int find_slot(hba_port_t *);

int do_write_to_disk(hba_port_t *,uint32_t ,uint32_t ,uint16_t ,char *);

int do_read_from_disk(hba_port_t *,uint32_t ,uint32_t ,uint16_t ,char *); // one more parameter ?
